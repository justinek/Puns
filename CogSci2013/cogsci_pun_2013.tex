...
\usepackage{etex}
\usepackage{color}\documentclass[10pt,letterpaper]{article}
\usepackage{multirow}
\usepackage{hyperref}
\usepackage{cogsci}
\usepackage{pslatex}
...
\title{\scalebox{0.9}{The Funny Thing About Incongruity: A Computational Model of the Humor in Puns}}
...
Here we explore the idea that puns such as these arise and are funny when they are due to noisy-channel processing.  Noisy channel models of sentence processing posit that language comprehension is a rational process that incorporates uncertainty about surface input to arrive at sentence-level interpretations that are globally coherent \cite{levy2008noisy, levy2009eye}. Comprehenders can thus consider multiple word-level interpretations (``viewpoints'') to arrive at more than one interpretation of a sentence, each coherent but potentially incongruous with each other. The notion of incongruity thus fits naturally into a noisy channel model of sentence comprehension.
...
\begin{figure}[t]
\includegraphics[width=80mm, height=60mm] {Plots/ambiguity_distinctiveness_ellipse.png}
...
\begin{figure}[t]
\includegraphics[width=90mm, height=60mm]{Plots/contour_plot_legend.png}
...
In this paper, we showed how a basic model of sentence comprehension can give rise to incongruous sentence interpretations with rich social and linguistic meaning.
...
%\sum_{\vec f} P(\vec w | m, \vec f) P(m) P(\vec f)= \sum_{\vec f} \bigg (P(m)P(\vec f)\prod_i P(w_i | m, f_i) \bigg)
%$$
We assume XXX for $P(m)$, and we assume a uniform probability that each words is in focus---hence $P(\vec f)$ is a constant.  As for $P(m | \vec w)$, note that it is driven in part by the semantic relationship between $m$ and $\vec w$ and in part by the prior probability of $m$, which we approximate using the unigram probability of the words $m_1$ and $m_2$.
From the generative model, 
\[
    P(w_i | m, f_i) = 
\begin{cases}
...
Researchers in artificial intelligence have argued that given the importance of humor in human communication, computers need to generate and detect humor in order to interact with humans more effectively \cite{mihalcea2006learning}. However, most of the work in computational humor has focused either on utilizing joke-specific templates and schemata \cite{binsted1996machine, kiddon2011s} or identifying surface linguistic features that strongly predict humorous intent \cite{mihalcea2006learning, semantic2010}. 
%The former type of studies is restricted to identifying jokes with a very specific format and structure, and the latter type falls short of testing or building upon more general theories of humor.
Our work moves beyond these two approaches and directly utilizes a model of general sentence comprehension to derive theory-driven measures of humor. This approach allows us to test the role of incongruity and apply similar modeling paradigms to explain other types of humor beyond homophone puns.
